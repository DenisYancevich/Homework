...
const kramarMethod = (matrix) => {
  if (!(matrix.length === ROWS_NUMBER)) {
    console.error("Provide 3 x 4 matrix, other not fits");
    return;
  }

  if (!matrix.every((row) => row.length === ROWS_NUMBER + 1)) {
    console.error(
      "Each row should contain 4 elements, 3 for unknown and 1 for result"
    );
    return;
  }
  ...
  for (const key in maxtrixList) {
    ...
    if (determinant === 0 && key === "main") {
      throw new Error(
        "Provided matrix is not fitting for Kramar method due to determinant equal 0"
      );
    } else {
